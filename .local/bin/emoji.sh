...
if hash bemenu 2> /dev/null; then
	read -r -a choice <<< "$(bemenu -n -m -1 -l 5 -p "emoji" <<< "$emoji_data")"
elif hash fuzzel 2> /dev/null; then
	read -r -a choice <<< "$(fuzzel -d -p "emoji" <<< "$emoji_data")"
elif hash wofi 2> /dev/null; then
	read -r -a choice <<< "$(wofi -d -p "emoji" <<< "$emoji_data")"
else
	echo "Could not find wofi or bemenu. Please install either one to use this script." 1>&2
	exit 1
fi

if [ ${#choice[@]} -gt 1 ]; then
	 echo -n "${choice[0]}" | wl-copy
fi
notify-send "$(wl-paste) on clipboard"
